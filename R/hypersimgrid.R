### hypersimgrid.R  (2006-10)
###
### Random hyperboloïd indexed field simulation on an user grid 
###                   by the mifieldsim method
###
###
...
hypersimgrid <- function(R,Ne=100,Nr=100,nbNeighbor=4,S){
...
##      S[,n] gives the hyperbolic point (S[1,n],S[2,n],S[3,n]) where the field must be 
##      simulated
...
##########################
##  S  : matrix of size 3xN
##      S[,n] gives the hyperboloïd point (S[1,n],S[2,n],S[3,n]) where the field has been 
##      simulated
##
...
if (is.function(R)==FALSE){
 stop("Message from hypersimgrid.R: R is not of valid type")}
 
if ((is.numeric(Ne)==FALSE)||(round(Ne)-Ne!=0)||(Ne<0)){
 stop("Message from hypersimgrid.R: Ne is not of valid type")}
 
if ((is.numeric(Nr)==FALSE)||(round(Nr)-Nr!=0)||(Nr<0)){
 stop("Message from hypersimgrid.R: Nr is not of valid type")}

if ((is.numeric(nbNeighbor)==FALSE)||(round(nbNeighbor)-nbNeighbor!=0)||(nbNeighbor<1)||(nbNeighbor>32)){
 stop("Message from hypersimgrid.R: nbNeighbor is not of valid type")}

if (nbNeighbor>Ne+1){
 stop("Message from hypersimgrid.R: nbNeighbor is too large or Ne is to small")}

#vaux <- S[1,]^2+S[2,]^2-S[3,]^2
#if ((is.matrix(S)==FALSE)||(nrow(S)!=3)||(vaux!=rep(-1,ncol(S))){
#stop("Message from hypersimgrid.R: S is not of valid type")}
...
Phi <- acosh(1+runif(Ne+Nr, min=0, max=1)*(Lim-1))
X <- cos(Theta)*sinh(Phi)
Y <- sin(Theta)*sinh(Phi)
Z <- cosh(Phi)
W <- rep(0,length(Z))
...
for (ts in 2:J){
    ...
    for (k in 1:(ts-1)){
       ...
       #
       if (abs(beta)>1){
        stop("message from hypersimgrid: R is not positive defined")
       } 
